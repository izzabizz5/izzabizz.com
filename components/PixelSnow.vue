...
vec3 getColor(float hashVal) {
  int idx = int(floor(hashVal * float(uColorCount)));
    ...
    if (cellHash < uDensity) {
      ...
      if (toIntersection > 0.0) {
        ...
        if (dist < flakeSize) {
          ...
          vec3 leafColor = getColor(h.y);
          gl_FragColor = vec4(leafColor * pow(vec3(intensity), vec3(uGamma)), 1.0);
          return;
        }
      }
    }
...
const setupFn = () => {
  ...
  // Convert colors to Vector3 array (max 6 colors)
  const colorVectors: Vector3[] = [];
  const colorsToUse = props.colors.slice(0, 6);
  for (let i = 0; i < 6; i++) {
    if (i < colorsToUse.length) {
      const c = new Color(colorsToUse[i]);
      colorVectors.push(new Vector3(c.r, c.g, c.b));
    } else {
      colorVectors.push(new Vector3(1, 1, 1));
    }
  }
  ...
  const material = new ShaderMaterial({
    ...
    uniforms: {
      ...
      uColorCount: { value: colorsToUse.length },
